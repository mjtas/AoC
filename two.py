...
# List to store game numbers that meet the conditions
result = []
...
for game in games:
    ...
    # Initialise flags for conditions
    blue_condition = green_condition = red_condition = True

    # Check conditions for each group
    for group in groups:
        blue_count = int(re.search(r'(\d+) blue', group).group(1)) if re.search(r'(\d+) blue', group) else 0
        green_count = int(re.search(r'(\d+) green', group).group(1)) if re.search(r'(\d+) green', group) else 0
        red_count = int(re.search(r'(\d+) red', group).group(1)) if re.search(r'(\d+) red', group) else 0

        # Update conditions
        blue_condition = blue_condition and (blue_count <= 14)
        green_condition = green_condition and (green_count <= 13)
        red_condition = red_condition and (red_count <= 12)

    # Check if all conditions are met and add to the result
    if blue_condition and green_condition and red_condition:
